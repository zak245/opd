...
2. **One door, in the same place, called the same thing.** "Filters and views", everywhere, holding every other filter, the columns, the density and the saved views in three labelled groups. Its badge says what it holds in words, and how many are on: `26 inside · 3 on`.
   **Whether there is a door is a property of the page, not of the width.** A page whose filters all fit the row — three at most, with nothing else behind them — shows them inline at every width, a phone included, and has no door anywhere. Every other page has the door at every width. A door that is there at 400 and gone at 1024 teaches a person where filters live and then takes it away.
