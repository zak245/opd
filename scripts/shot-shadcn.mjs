// The five screens the owner asked to see, on shadcn's components.
//   node scripts/shot-shadcn.mjs shots/shadcn 1440 900
import puppeteer from "puppeteer-core"
import { mkdirSync } from "node:fs"
const [dir = "shots/shadcn", w = "1440", h = "900"] = process.argv.slice(2)
...
for (const theme of ["light", "dark"]) {
  ...
  for (let i = 0; i < n; i++) {
    // At phone width the row menu may be under the sidebar sheet; a miss is not a failure.
    try { await page.click(`[data-probe="${i}"]`, { timeout: 3000 }) } catch { continue }
    await wait(300)
  ...
  await wait(800); await shot("2-people-pane")
  ...
  await wait(700); await shot("5-dialog")
  await page.close()
}
await browser.close()
