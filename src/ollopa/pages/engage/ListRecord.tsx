...
export function ListRecord({ session, id }: { session: Session; id?: string }) {
  ...
  /** The whole record, in the layout every record uses: masthead, key fields, sections, rail. */
  return (
    <DoorGroup>
    <RecordPage
      family={list.kind === "people" ? "people" : "companies"}
      back={{ label: "Lists", href: href("/ollopa/lists") }}
      title={{
        value: list.name,
        onRename: isOwner ? (v: string) => { engage.patchList(session.business, list.id, { name: v }); say(`Renamed to ${v}`) } : undefined,
      }}
      chips={<>
      ...
      fields={[
        { key: "count", label: list.kind === "people" ? "People" : "Companies", value: `${n(count)} · ${touchEstimate(count, session.business)}` },
        { key: "owner", label: "Owner", value: list.owner },
        { key: "visibility", label: "Visible to", value: list.visibility === "everyone" ? "Everyone" : "Only you" },
        {
          key: "refreshed",
          label: list.mode === "segment" ? "Refreshed" : "Updated",
          value: list.mode === "segment" && list.lastRefreshed ? ago(list.lastRefreshed) : ago(list.updated),
        },
        ...(list.newThisWeek > 0 ? [{ key: "new", label: "New this week", value: n(list.newThisWeek) }] : []),
      ]}
      ...
      main={{
        kind: "sections",
        sections: [
          ...(list.kind === "people" || list.mode === "segment" || list.feeds.some((f) => f.auto) || watch ? [{
            id: "list-standing",
            title: "What this list is doing on its own",
            children: (
      ...
      {list.mode === "segment" && (
        <div className="mt-3">
          ...
          {
            id: "list-members",
            title: list.kind === "people" ? "People in this list" : "Companies in this list",
            count: count,
            action: count > SEARCH_OVER ? (
              <Input
                data-page-search
      ...
      side={[]}
      doors={[{ id: "list.history", label: "History", count: list.history.length, openByDefault: d.level("detail.history") === 1, content: (
