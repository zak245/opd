...
export function ForecastTab({ session, report, drillTo, onDrill, onSubmitPanel, onRecords, mine }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="t-body flex flex-wrap items-center gap-x-3 gap-y-1">
        <span className="font-medium">{report.periodLabel}</span>
        <span className="text-muted-foreground">· {report.deadline}</span>
        <span className="rounded-full border px-2 py-0.5 t-small text-muted-foreground">{report.scopeLabel}</span>
        {drillTo && (
      ...
      <ul className="grid grid-cols-1 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        {report.categories.map((c) => (
          <li key={c.name} className={cn("surface-raised rounded-lg border p-3", c.counted && "border-foreground/30")}>
            ...
            <div className="t-section mt-1">{money(c.amount, cur)}</div>
            <p className="t-small mt-1 text-muted-foreground">{c.definition}</p>
          </li>
        ))}
      </ul>
      ...
      <div className="surface-raised rounded-lg border p-3">
        ...
        {report.goal === null ? (
          ...
          <>
            ...
            <p className="t-small mt-1 text-muted-foreground">Goal set for {report.goalSource}.</p>
          </>
        )}
      </div>
      ...
      <div className="surface-raised rounded-lg border p-3">
        <div className="flex flex-wrap items-baseline gap-x-6 gap-y-1">
          <div>
            ...
            <div className="t-title">{submitted === null ? "Not submitted" : money(submitted, cur)}</div>
          </div>
          <div>
            <div className="t-small text-muted-foreground">Predicted</div>
            <div className="t-title">{report.predicted === null ? "—" : money(report.predicted, cur)}</div>
          </div>
          {report.drivers.length > 0 && (
            <button
              type="button"
              onClick={() => onRecords("drivers")}
              className="t-body rounded underline decoration-dotted underline-offset-4 hover:decoration-solid focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
            >
                ...
                {report.repRows.map((r) => (
                  <tr key={r.user} className="border-b last:border-b-0">
                    ...
                    <td className="px-3 py-1.5 text-right">
                      <button type="button" className="t-small underline" onClick={() => onDrill(r.user)}>Their deals</button>
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <section className="surface-raised rounded-lg border">
      ...
      {deals.length > sorted.length && (
        <p className="border-t px-3 py-1.5 t-small text-muted-foreground">Showing {sorted.length} of {deals.length}, by category then risk.</p>
      )}
    </section>
  )
}
    ...
    >
      ...
      {report.last && (
        <section className="rounded-md border p-3">
          <h3 className="t-small font-medium uppercase tracking-wide text-muted-foreground">Last cycle</h3>
          <p className="mt-1">
      ...
      {report.repRows.length > 0 && (
        <section className="mt-4">
          <h3 className="t-small font-medium uppercase tracking-wide text-muted-foreground">The team roll-up</h3>
          <ul className="mt-1 space-y-0.5 t-small">
            {report.repRows.map((r) => (
      ...
      <section className="mt-4">
        <h3 className="t-small font-medium uppercase tracking-wide text-muted-foreground">By category</h3>
        <table className="mt-2 w-full t-small">
          <thead>
              ...
              <tr key={c.name} className="border-b last:border-b-0 align-top">
                ...
                <td className="py-2 text-right">
                  <Input
                    ...
                    className="ml-auto h-7 w-28 px-1.5 text-right t-small tabular-nums"
      ...
      <section className="mt-4">
        <Label htmlFor="forecast-note" className="t-small">Your judgement — why the number is what it is</Label>
      ...
      <section className="mt-4 border-t pt-3">
        <h3 className="t-small font-medium uppercase tracking-wide text-muted-foreground">What changed since last time</h3>
        {changes.length === 0 ? (
          ...
          <ul className="mt-1 space-y-0.5 t-small text-muted-foreground">{changes.map((c) => <li key={c}>{c}</li>)}</ul>
        )}
      </section>
