...
export function AppShell({ session, page, title, children, defaultCollapsed }: { session: Session; page: Page; title: string; children: ReactNode; defaultCollapsed?: boolean }) {
  ...
  return (
    <div className="flex h-screen bg-background text-foreground">
      ...
      <aside className={cn("surface-chrome hidden shrink-0 flex-col border-r border-r-[color:var(--border-strong)] md:flex", collapsed ? "w-14" : "w-56")}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className={cn("surface-chrome flex shrink-0 items-center gap-2 border-b border-b-[color:var(--border-strong)] px-3 sm:gap-3 sm:px-4", canAdd || added ? "h-auto min-h-14 flex-wrap py-1.5 md:h-14 md:flex-nowrap md:py-0" : "h-14")}>
        ...
        <div className="relative flex min-h-0 flex-1">
          <main id="ollopa-main" className="surface-page relative min-h-0 flex-1 overflow-y-auto pb-16 md:pb-0">{children}</main>
          <Beside session={session} pageTitle={title} />
        </div>

        <nav className="surface-chrome fixed inset-x-0 bottom-0 z-30 flex border-t border-t-[color:var(--border-strong)] md:hidden" aria-label="Pages">
          {bottom.map((p) => {
            const nav = navItem(p)!
            return (
              <a
