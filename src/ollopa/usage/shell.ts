...
export const shellItems: UsageItem[] = [
  ...
  { id: "nav.workflows", page: "home", area: "shell", label: "Sidebar: Workflows", weekly: { marketer: 55, admin: 30 }, overrides: { ridgeline: { marketer: 70, admin: 25 }, halyard: { admin: 10 }, fathom: { admin: 5 } }, note: "Routing rules and form handling: M4 and M5 are the marketer's work, so the marketer holds the page as well as the admin. The Agency and Founder-led profiles leave it out of the sidebar; it opens by link and offers \"Add to sidebar\". Growth-gated, shown on Starter with a lock and the plan name." },
  { id: "nav.requests", page: "home", area: "shell", label: "Sidebar: Requests", weekly: { admin: 45 }, overrides: { halyard: { admin: 50 }, ridgeline: { admin: 30 }, fathom: { admin: 8 } }, note: "The admin's intake queue, both kinds in one place: change requests and upgrade requests. 19§4.1 measures the same work at 2.1 hours a week when it lives in Slack. The Founder-led profile leaves it out; the second upgrade request in a week is the signal that exposes it." },
  ...
  { id: "top.credits", page: "home", area: "shell", label: "Top bar: credits pill (balance, weekly burn, run-out date)", critical: true, weekly: { sdr: 30, ae: 15, marketer: 20, cs: 5, admin: 55 }, overrides: { fathom: { sdr: 40, admin: 60 } }, note: "Same numbers as Settings › Credit balance and burn rate; the pill is where most of those looks happen. It opens X-credits over the current page on its own channel, like the bell, and the run-out date joins the pill text whenever it falls inside the billing period." },
  ...
  { id: "shell.toast", page: "home", area: "shell", label: "Interrupting toast (bounce guard, a second approval over the threshold, credits low)", critical: true, weekly: { sdr: 15, ae: 5, marketer: 3, cs: 2, admin: 35 }, overrides: { fathom: { sdr: 40, admin: 50 }, halyard: { sdr: 30, admin: 55 }, ridgeline: { sdr: 3, admin: 15 } } },
  ...
  { id: "bell.sync-error", page: "home", area: "shell", label: "Notification: CRM sync error (digestible, cadence set on the integration)", weekly: { admin: 35 }, overrides: { fathom: { admin: 5, sdr: 5 }, halyard: { admin: 30 }, ridgeline: { admin: 20 } }, note: "Digestible, not interrupting: nothing is sending because of it and the records are still there to fix. The review cadence — instantly, daily or weekly, daily by default — is a choice on the integration. The failure count stays level one on Home's health strip and the Settings › Integrations row." },
  { id: "bell.signal-fired", page: "home", area: "shell", label: "Notification: a signal fired, grouped by signal per day", weekly: { sdr: 30, ae: 12, marketer: 20 }, overrides: { ridgeline: { sdr: 12, marketer: 35 }, fathom: { sdr: 25, admin: 25 } }, note: "Job changes, funding, hiring, product usage. Opens People filtered to the signal, so the row is a way into work rather than a fact to remember." },
  { id: "bell.handoff", page: "home", area: "shell", label: "Notification: hand-off received", weekly: { ae: 5, cs: 12 }, overrides: { ridgeline: { cs: 20 } }, note: "One per closed-won deal, never grouped: a hand-off is a single object with a person's name on it." },
  { id: "bell.account-signal", page: "home", area: "shell", label: "Notification: account signal (health band change or expansion)", weekly: { ae: 10, cs: 25 }, overrides: { ridgeline: { ae: 20, cs: 60 } }, note: "Grouped per account per day and never interrupting: the routed owner and the due date are on the row, and the work itself is on Accounts." },
