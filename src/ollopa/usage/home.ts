...
export const homeItems: UsageItem[] = [
  ...
  // Replies
  { id: "home.replies.hot", page: "home", area: "Replies", label: "Interested and question replies", weekly: { sdr: 85, ae: 45, marketer: 3, cs: 2, admin: 4 }, overrides: { fathom: { admin: 55 }, ridgeline: { sdr: 40, ae: 25 } }, note: "Includes the Reply action on each row; Reply opens Inbox with that thread and its composer, not a second composer on Home." },
  ...
  // Agents
  { id: "home.agents.approvals", page: "home", area: "Agents", label: "Waiting for your approval", critical: true, weekly: { sdr: 55, ae: 25, marketer: 5, cs: 3, admin: 45 }, overrides: { fathom: { admin: 70, sdr: 60 }, halyard: { admin: 60, sdr: 55 }, ridgeline: { sdr: 20, ae: 20, cs: 5, marketer: 6 } }, note: "Includes Approve and Decline on each row. Only irreversible or costly actions queue: sending email, spending above a cap, changing a deal stage. Research, scoring and saved drafts are logged, not queued, so the marketer and CS see few. The owner of the object approves; the admin may approve for anyone (rule 7)." },
  { id: "home.agents.credits", page: "home", area: "Agents", label: "Credits each approval will spend", critical: true, weekly: { sdr: 55, ae: 25, marketer: 5, cs: 3, admin: 45 }, overrides: { fathom: { admin: 70, sdr: 60 }, halyard: { admin: 60, sdr: 55 }, ridgeline: { sdr: 20, ae: 20, cs: 5, marketer: 6 } }, note: "Cost sits on the same row as the Approve button, and the batch header carries the total; never behind the detail door (rule 5, rule 7)." },
  { id: "home.agents.second-approval", page: "home", area: "Agents", label: "Over the threshold: needs a second approval from the admin", critical: true, weekly: { sdr: 6, ae: 2, marketer: 8, cs: 1, admin: 20 }, overrides: { fathom: { admin: 8, sdr: 4 }, halyard: { admin: 25, sdr: 12 }, ridgeline: { marketer: 10, admin: 12 } }, note: "Above the workspace threshold (a Settings item; default 1,000 recipients or 500 credits in one action) the owner's approval is not enough. The requester sees the row as waiting and who it waits on; the admin sees it to decide (PLAN, 14 Sep 2026)." },
  ...
  { id: "home.agents.week", page: "home", area: "Agents", label: "What agents did this week", weekly: { sdr: 15, ae: 6, marketer: 8, cs: 2, admin: 25 }, overrides: { fathom: { admin: 40 }, halyard: { admin: 35 } }, note: "A door. It carries the low-cost reversible work that is logged rather than queued (research, scoring, drafts saved not sent). The full ledger is the Agents page." },
