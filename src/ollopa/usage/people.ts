...
export const peopleItems: UsageItem[] = [
  ...
  { id: "people.views.needs-enrichment", page: "people", area: "Search and views", label: "Needs enrichment: a shipped view, with its count", weekly: { sdr: 15, ae: 5, marketer: 8, admin: 25 }, overrides: { halyard: { admin: 35, sdr: 22 }, fathom: { admin: 20, sdr: 14 } }, note: "Two or more of email, phone, job title, company name and last enrichment date missing or older than twelve months. It ships rather than being rebuilt in each workspace, because the data-quality pass is the same work everywhere and its count is what says whether the pass is needed. Its rows feed the enrichment panel and the import wizard." },
  ...
  { id: "people.f.persona", page: "people", area: "Filters", label: "Persona", weekly: { sdr: 30, ae: 10, marketer: 35, admin: 6 }, overrides: { ridgeline: { sdr: 20, marketer: 40 }, fathom: { sdr: 12, admin: 12 } }, note: "A level-one chip for the SDR and the marketer. The definitions are declared once in Settings › Signals, scoring and personas and read here and in Find people, instead of being retyped as a title filter every morning. Removed at a business that defines no personas." },
  { id: "people.f.score", page: "people", area: "Filters", label: "Score: bands, and above the MQL threshold", weekly: { sdr: 30, ae: 25, marketer: 55, admin: 10 }, overrides: { ridgeline: { marketer: 60, sdr: 35 }, fathom: { sdr: 2, admin: 2 } }, note: "The published threshold and the date and person who published it print beside the chip: a score whose threshold moved yesterday and does not say so is a filter that quietly means something else." },
  ...
  { id: "people.col.score", page: "people", area: "Columns", label: "Score", weekly: { sdr: 25, ae: 12, marketer: 45, admin: 8 }, overrides: { ridgeline: { marketer: 50, sdr: 30 } }, note: "Stamped with its model version and date when the person is routed, and never recomputed after that, so the number in the column is the number the routing used." },
  ...
  { id: "people.job-change-update", page: "people", area: "Row actions", label: "Job change: confirm and update, or create a new contact", weekly: { sdr: 12, ae: 6, marketer: 3, admin: 4 }, overrides: { ridgeline: { sdr: 30, ae: 12 }, fathom: { sdr: 18, admin: 18 } }, note: "Shown only while the job-change signal is live on that person, removed otherwise (rule 4). Each option states what it keeps, and one line says whether the new employer is already an account, with its owner and open deals." },
  ...
  { id: "people.enrich-panel", page: "people", area: "Bulk actions", label: "Enrichment panel: what to reveal, the estimate, the total and the balance after", weekly: { sdr: 45, ae: 15, marketer: 10, admin: 10 }, overrides: { fathom: { admin: 80, sdr: 55 }, halyard: { sdr: 30, admin: 30 } }, note: "The selection bar opens the panel rather than spending from the bar: the choice of what to reveal changes the price per row, so the two belong on the same surface (rule 5). The same panel opens from the Needs enrichment view and from the import wizard." },
  { id: "people.dnc-exclusion", page: "people", area: "Bulk actions", label: "Do-not-call rows excluded from a mobile reveal, with the count and an override", critical: true, weekly: { sdr: 30, ae: 8, marketer: 4, admin: 10 }, overrides: { fathom: { admin: 60, sdr: 40 } }, note: "A mobile for a do-not-call record is charged and cannot be called. The exclusion is stated with its count and can be overridden on purpose, never applied silently (rule 7)." },
  ...
  { id: "people.import", page: "people", area: "Page", label: "Import CSV (opens the import wizard page)", weekly: { sdr: 4, ae: 2, marketer: 15, admin: 10 }, overrides: { halyard: { sdr: 12, admin: 30 } }, note: "A page, not a drawer: the wizard has its own steps, its own ten-row credit trial and a job record at the end, and a page is what carries a deep link back into it (spec 17)." },
