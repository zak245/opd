...
| The second axis: chains | Progressive disclosure judges a screen at a moment; the chains research (knowledge-base/13-chains-of-work.md) judges the person moving through objects and time. Three mechanics built in the shell and adopted by every page (BUILD-CHAINS.md): a related object opens beside the page in a pane that carries the record's usage-model level one and the chain's actions, with next and previous over the list it came from; a trail built only from the person's own moves, with the origin page kept mounted and the return landing on the lit row; related lists live inside the object. Actions in a pane land on the row behind through one shared edits store, with undo. A second nine-question scorecard (the chain card, in BUILD-CHAINS.md) is applied beside the disclosure card; an independent reviewer walked nine chain sets over five rounds until every chain scored 18 on the chain card and 17 on disclosure, the 17 being the analytics point no demo can have (REVIEW-CHAINS.md) | Decided 21 Sep 2026 with the owner: build the better version and iterate until both dimensions are full |
| The look and the layouts | After three failed attempts at an invented surface and elevation system, the look comes from shadcn components as shipped (DESIGN.md §4); our rules keep disclosure, chains, actions, colour meaning and type. Page layouts are a system (LAYOUTS.md and `src/ollopa/layouts/`): ten page types, one set of parts, one overlay rule, a visible affordance for every interaction, and one behaviour per type at three widths; a page declares its type and fills its parts. The see harness (`npm run see`) shoots every page and interaction state so visual work is judged from images, never from code | Decided 23 Sep 2026 with the owner: build as a system, reusable and exposable in other contexts |
