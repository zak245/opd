...
|---|---|---|---|---|
| Founder-led outbound | Inbox, Campaigns, **Accounts, Reports, Workflows, Requests** | all | Two founders and one SDR: replies are few enough in the first weeks that Home carries them, nobody is running campaigns, nobody has a customer to keep yet, there is no team to report on, nothing routes itself yet, and nobody is asking the admin for a field. Workflows is also Growth-gated and Fathom is on Starter, so the page shows with a lock and the plan name and has nothing to route | Inbox: the first reply. Campaigns: the first audience or campaign anyone creates. Accounts: the first deal reaching Closed won. Reports: the first full week with more than ten sends. Workflows: the first workflow anybody creates. Requests: the second upgrade request in a week |
| Separated sales team | none | — | Every seat exists and every page has a seat that lives in it | — |
| Agency | Campaigns, Accounts, **Workflows** | all | The workspaces hold outbound run for clients; campaign, account and routing work lives in the client's own systems | Campaigns: the first audience. Accounts: the first deal reaching Closed won. Workflows: the first workflow anybody creates |
| Product-led growth | **Sequences** for the account executive and the admin; **Lists** for the admin | the seats named | Two sequences, run by one SDR. At Ridgeline the AE touches Sequences 10% weekly, the admin 5%, and the admin touches Lists 15% — all three under the line (`sequences.ts`, `lists.ts`). The **marketer keeps Lists**: segments are the marketer's daily work at a product-led business, and taking them away would have been the profile overriding the seat | Sequences: a sequence created, or a step due for you. Lists: a list created |

Three corrections are recorded in that table rather than smoothed over. The Founder-led row used to read "Inbox, Campaigns" here and "Campaigns, Accounts, Reports, and Inbox until the first reply" in the shell spec; the **wider list wins**, because a profile that leaves out less than the evidence supports is a profile that puts a page in a founder's sidebar they will not open for a month, and every one of the six has a named signal that brings it back. The Product-led row used to read "Sequences, Lists — admin only", which was two mistakes in four words: it left Sequences in an AE's sidebar at 10% weekly, and it took Lists from the seat that uses them most. And **Workflows and Requests were missing from both profiles** here after the two pages arrived on 15 September: `P-workflows` carries the sidebar column SEP, PLG and `P-requests` carries SEP, AG, PLG in IA-MAP 2.11, which leaves Workflows out of the Founder-led and the Agency sidebar and Requests out of the Founder-led one. This table and [00 Shell](00-shell-signin-palette-notifications.md) §3.2 now carry the same two lists; PLAN.md's Founder-led decision resolved the five pages that had already been written down, and the node table settles the sixth.
...
| Fathom | Once, in the past | Just me and two others; we all do everything. Founder-led outbound, so Inbox, Campaigns, Accounts, Reports, Workflows and Requests were left out, each with its own signal; Inbox has since come back for two weeks on seven replies, and Workflows is locked at Starter as well as left out. The Starter seat limit of three was stated on this page before anyone was invited |
| Halyard | Monthly | Every client workspace gets its own answers, prefilled from the last one. Agency each time |
| Ridgeline | Once, in the past | Grow existing accounts; 6 to 25; all four jobs. Product-led growth, so Sequences is left out for the account executive and the admin and Lists for the admin, while the marketer keeps both |
...
| All four businesses covered | This spec and the shell spec carried two different lists of what Founder-led leaves out, and neither carried Workflows or Requests | The wider list wins: Inbox, Campaigns, Accounts, Reports, Workflows and Requests, each with a named signal — first reply, first audience or campaign, first deal reaching Closed won, first full week over ten sends, first workflow created, second upgrade request in a week. Agency leaves out Workflows too, and both lists now match spec 00 §3.2 |
| All roles covered | "Sequences, Lists — admin only" left Sequences in a Ridgeline AE's sidebar at 10% weekly and took Lists off the marketer, whose segments are that business's daily work | The row now names the seats: Sequences for the account executive and the admin, Lists for the admin only. A profile may subtract from a seat's sidebar; it may never override what the seat is for |
