...
The page connects an outside system to the workspace: a CRM (Salesforce or HubSpot), a calendar (Google or Microsoft 365), Slack, an enrichment provider, or a webhook. For a CRM it also decides what syncs, how fields map, and what ollopA may write, delete or merge in the other system. Then it hands over — and the destination depends on the kind, because one object belongs in one container (rule 5). A **CRM, a calendar, Slack or an enrichment provider** hands over to the integration's own page, where the connection lives for the rest of its life: status, error log, mapping and rules, all editable in place. A **webhook** hands over to Settings › API, webhooks, MCP and CLI › the subscription, because that is where the delivery contract, the delivery log and the reconciliation endpoint live (spec 17). The wizard creates either one; it does not own either one.
...
The CRM group also carries a third answer, which is not a card: **"ollopA is our CRM."** Declaring it is a decision, not an absence — it takes the CRM row out of Home's set-up list, which at Fathom empties that list so the door goes rather than nagging for ever, and it is reversible from the same place. Before this, a workspace that had decided never to connect a CRM carried a permanent to-do about it, which is a door onto nothing (rule 4). At Halyard that rule is per client workspace, so each one has its own CRM and its own cards. The card chosen decides the step list: six steps for a CRM, three for anything else.
...
**Step 2: Authorise.** HubSpot, Google, Microsoft 365, Slack: one button, "Sign in to HubSpot", opening the provider's consent screen. Salesforce: production or sandbox as two radios, then **the requirement above the button, not after the consent screen**: "The sync user needs create, read and edit on Accounts, Contacts, Leads, Opportunities and User Roles, and API Enabled under System Permissions. Salesforce Essentials cannot connect." Then "Sign in to Salesforce as the sync user", with one line saying the sync user should be a shared account that stays active. A requirement discovered after the sign-in is a requirement discovered too late, and the person who has to grant it is often not the person at the keyboard (rule 7). No package to install, no separate setup app. Enrichment: key field and "Check the key". Webhook: URL, secret, "Send a test event", response shown in place. The result is text on the step: "Connected as ollopa-sync@meridian.com. Token valid until 12 Mar 2027."

**Step 3: Choose what syncs.** CRM: four rows (contacts, companies, deals, activities), four radios each (both ways, pull only, push only, off). On Starter the "both ways" radio carries the plan name and opens the upgrade panel, and the row still works one-way; the limit is on the radio, where the choice is made. Custom CRM objects are a fifth row on Growth and below, with a lock and "Scale", placed in the object list so nobody maps fields for an object they cannot sync. Defaults: both for contacts and companies, pull for deals, push for activities. Each row has one plain-words line ("ollopA will create and update Salesforce contacts and take updates back"). Activities have one door, "Which activities to push (emails, calls, tasks, meetings)", expanded in place. Calendar: which calendars, busy time only or event detail. Slack: five events (reply received, meeting booked, deal moved, agent needs approval, sync error), each with a channel. Enrichment: which fields it may fill and its place in the provider order, shown with the other providers. Webhook: which events — and one line under the list: **"Approvals are not webhook events. A webhook fires on what happened, never on what needs deciding; approvals arrive in the app's queue, in the daily digest, and in Slack, which carries a link."** A surface with no reply path cannot carry a decision, and an auto-approver built against a webhook has zero review capacity, which is rule 7's corollary at its worst. Saying so on the event list is the only way the boundary is not discovered by building against it.

**Step 4: Map fields.** CRM only. One tab per object chosen, labelled with its counts: **"Contacts · 14 mapped, 9 suggested, 1 required unmapped"**. Rows: ollopA field, direction, CRM field, write rule (fill empty, overwrite), and a **state** — mapped, **suggested**, or edited. Standard fields are pre-mapped and marked **suggested**, never presented as decided: a guess shown as a decision is what rule 6 objects to. Saving the step confirms every suggestion and says so ("9 suggested pairs confirmed"); editing one marks it edited and it keeps that state. CRM-required fields without a mapping are listed first with a text mark, not colour alone. "Add a field pair" adds a row. Deals also carry stage mapping in the same tab, one to one, with "Match stages by name". The write rule sits in the row of the pair it governs, never on another step. A search box filters both columns; a door "Show 42 unmapped Salesforce fields" holds the long tail.
...
Level one: heading with name and kind; a status strip with status word, last sync, next sync, records synced today, errors today, and "Pause syncing" or "Resume syncing". When errors today is above zero, the errors sit under the strip — **grouped by cause, largest group first**, one row per cause with its count, a one-line cause, and **"Retry these 340"**. A group expands in place to its records (when, object, record as a link, direction, message, suggested fix, attempts). The nine named causes are the ones the wizard's own rules can produce: duplicate email, missing required field, read-only field, picklist value rejected, record deleted in the CRM, owner not mapped, API limit reached, token expired, and validation rule. Date and object filters remain as a control, and "Export as CSV" stays.

Flat and newest-first was the wrong shape: 340 rows of one cause is one decision, not 340, and a person who has to scroll to learn that will not learn it. Grouping by cause is also exactly the treatment a failing webhook subscription gets in the developer area — one pattern, two places, so nobody has to learn a second one. Message and fix are printed in the row, never on hover. Then "Disconnect" with its consequence beside it in one line: "Stops syncing. Records already in Salesforce stay. Records pulled into ollopA stay and lose their link."
...
| Field mapping | 14 contact, 9 company, 6 deal fields · 9 suggested · 1 required Salesforce field unmapped | Step 4's tabs with their states, editable, "Save", "Apply mapping to existing records". Removing a pair states its consequence at the pair |
...
| Retry a group | The control states the count and the batching before the click: "Retry 340 duplicate errors · 100 at a time, about 4 passes". Rows show "Retrying…", then leave the group or come back with a new cause; **the group's count after the run is the confirmation**, so nothing needs a toast |
| Edit a door and Save | Saved in place; summary line updates. **Removing or retyping an existing pair** raises its consequence at the pair, before Save: "Removing this pair can stop Contacts syncing until the next full pull. Run a full pull after saving?" — with the pull offered there. Editing a mapping is the most destructive thing on this page, and it used to carry an applies-to line and no warning (rule 7) |
...
| State | Shown |
|---|---|
| No integrations | Settings › Integrations shows the seven cards with their one-line descriptions. This is Fathom's normal state for a CRM, not an error, and once "ollopA is our CRM" is declared it is not even a pending decision: **"ollopA is your CRM. It is holding your contacts, companies and deals. Connect Salesforce or HubSpot if that changes."** The declaration is reversible from the same place, and it is what empties Home's set-up list rather than leaving a to-do that can never be completed |
...
|---|---|
| Fathom | No CRM at all, **and that is declared rather than outstanding**: the founder answered "ollopA is our CRM" on the CRM group, which removed the CRM row from Home's set-up list. ollopA is the system of record here, so the two CRM cards sit in the list unconnected like any other card and nothing reads "not on your plan", because every plan syncs. What is locked is the webhook card (Growth), with the plan name and the monthly total on it. The integration page is a calendar and an enrichment provider, with no error table |
...
| wiz.declare-no-crm | Wizard | "ollopA is our CRM": a declarable answer on the CRM group | | | | | 3 | 12 | 2 | 1 | |
| wiz.sf-permissions | Wizard | What the sync user must be able to do, above the sign-in button | | | | | 2 | 0 | 5 | 0 | yes |
...
| wiz.suggested-mappings | Wizard | Pre-mapped rows marked suggested, counted in the tab label, confirmed by saving | | | | | 5 | 3 | 22 | 3 | |
| wiz.no-approval-events | Wizard | Approvals are not webhook events, and where they do arrive | | | | | 2 | 1 | 1 | 6 | yes |
...
| int.handover | Integration page | Where the wizard hands over, named by kind | | | | | 4 | 3 | 10 | 4 | |
| int.status | Integration page | Status, last and next sync, records and errors today | 3 | 3 | | 2 | 40 | 5 | 25 | 30 | yes |
| int.errors | Integration page | Errors grouped by cause, each with a count and a one-line cause | | | | | 30 | 3 | 25 | 25 | |
| int.retry | Integration page | Retry a group, with its count and how it batches | | | | | 20 | 2 | 18 | 15 | |
...
| int.mapping-consequence | Integration page | What removing or retyping a pair does, stated at the pair before Save | | | | | 8 | 2 | 8 | 6 | yes |
...
| Every action has an outcome | "Retry all" said neither how many nor how long | The control states the count and the batching before the click, and the group's count after the run is the confirmation |
| Decision-critical visible | Removing a field pair had an applies-to line and no warning | Its consequence is raised at the pair before Save, with the full pull offered there. It is the most destructive thing on this page |
| Decision-critical visible | Salesforce's permission requirement was discovered after the consent screen | Stated above the sign-in button, including that Essentials cannot connect |
| Doors labelled by content | A flat, newest-first error table made 340 rows of one cause look like 340 decisions | Grouped by cause, largest first, each group a count and a one-line cause with "Retry these 340"; a group expands in place. The same treatment the developer area gives a failing webhook |
| Stable, user-controlled disclosure | Pre-mapped pairs were presented as decided | Marked **suggested**, counted in the tab label, confirmed by saving the step with a line saying so, and marked edited when touched |
| Role gaps explain themselves | Fathom carried a permanent CRM to-do for a decision already made | "ollopA is our CRM" is a declarable answer that removes the row from Home's set-up list and is reversible from the same place |
| Two levels maximum | A webhook subscription had two homes: this page and the developer area | The hand-over sentence names the destination by kind. A CRM, a calendar, Slack and an enrichment provider hand over here; a webhook hands over to Settings › API, webhooks, MCP and CLI, where its contract lives |
| Decision-critical visible | Nothing stopped an integrator wiring an approval auto-approver against a webhook | One line on the event list: approvals are not webhook events, and where they do arrive |
